...
def main():
    ...
    for i in range(0,7):
      hma = HFmaior(L[i])
      print(f"Resultado de HFmaior({L[i]}) = {hma}")
      hme = HFmenor(L[i])
      print(f"Resultado de HFmenor({L[i]}) = {hme}")
...
def HFmaior( n ):
    ...
    Recebe um inteiro positivo e retorna o valor da soma 1 + 1/2 +...+ 1/n
    (Soma realizada nesta ordem)
    '''
    s = Fraction()
...
def HFmenor( n ):
    ...
    Recebe um inteiro positivo e retorna o valor da soma 1/n + 1/(n-1) +...+ 1
    (Soma realizada nesta ordem)
    '''
    s = Fraction()
    for i in range(1,n+1):
